...
passport.use(new LocalStrategy(async (username, password, done) => {
  try {
    ...
    if (!user.is_admin && user.runs_remaining <= 0) {
      return done(null, false, { message: 'Account locked - no runs remaining. Contact administrator.' });
    }
...
// Get my stats (any authenticated user)
app.get('/api/my-stats', requireAuth, (req, res) => {
  res.json({
    runs_remaining: req.user.runs_remaining,
    runs_total: req.user.runs_total,
    runs_used: req.user.runs_total - req.user.runs_remaining
  });
});
...
app.put('/api/users/:id/runs', requireAdmin, async (req, res) => {
  try {
    ...
    // If runs_remaining > 0 and account was locked, unlock it
    if (runs !== undefined && runs > 0) {
      // Account will be accessible on next login
    }
    res.json({ success: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
